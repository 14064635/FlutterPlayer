...
public class NewWrappedSoundPool extends Player {
    ...
    static {
        soundPool.setOnLoadCompleteListener(new SoundPool.OnLoadCompleteListener() {
            public void onLoadComplete(SoundPool soundPool, int sampleId, int status) {
                ...
                NewWrappedSoundPool loadingPlayer = soundIdToPlayer.get(sampleId);
                if (loadingPlayer != null) {
                    ...
                    synchronized (urlToPlayers) {
                        List<NewWrappedSoundPool> urlPlayers = urlToPlayers.get(loadingPlayer.url);
                        for (NewWrappedSoundPool player : urlPlayers) {
     ...
     * it has been loaded.
     */
    private static Map<Integer, NewWrappedSoundPool> soundIdToPlayer = Collections.synchronizedMap(new HashMap<Integer, NewWrappedSoundPool>());
     ...
     */
    private static Map<String, List<NewWrappedSoundPool>> urlToPlayers = Collections.synchronizedMap(new HashMap<String, List<NewWrappedSoundPool>>());


    private final MediaplayService ref;
    ...
    NewWrappedSoundPool(MediaplayService ref, String playerId) {
        this.ref = ref;
    ...
    void release() {
        ...
        if (this.soundId != null && this.url != null) {
            synchronized (this.urlToPlayers) {
                List<NewWrappedSoundPool> playersForSoundId = this.urlToPlayers.get(this.url);
    ...
    void setUrl(final String url, final boolean isLocal) {
        ...
        synchronized (urlToPlayers) {
            this.url = url;
            List<NewWrappedSoundPool> urlPlayers = urlToPlayers.get(url);
            if (urlPlayers != null) {
                // Sound has already been loaded - reuse the soundId.
                NewWrappedSoundPool originalPlayer = urlPlayers.get(0);
